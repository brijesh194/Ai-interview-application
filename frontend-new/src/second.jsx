...
import {
  ...
  BrainCircuit, MessageSquare, UserCheck, Activity, Globe, BarChart3, Clock, AlertTriangle, Layers
} from 'lucide-react';
...
function App() {
  ...
  const speak = (text) => {
    if (!text || !isInterviewStarted) return; // 🔥 Check added
  ...
  useEffect(() => {
    let proctorInterval;
    // Agar interview started hai TABHI interval chalega
    if (isInterviewStarted) {
      proctorInterval = setInterval(runProctoring, 2000);
    }
    // Cleanup: Jab component unmount ho ya interview stop ho, interval clear ho jaye
  ...
  const handleUpload = async () => {
    ...
    } catch (err) { alert("Server Error! Backend check karo."); }
    setLoading(false);
  };
  ...
  const nextQuestion = async () => {
    ...
    } catch (err) { console.error("Next Q Error:", err); }
  ...
  const finishInterview = async () => {
    ...
    // 🔥 BACKEND STOP LOGIC
    // 1. AI ka bolna turant band karo
    window.speechSynthesis.cancel();

    // 2. Interview state ko false karo taaki Intervals ruk jayein
    ...
    try {
      ...
      setShowResult(true); // Dashboard dikhao
    } catch (err) {
      alert("Analysis failed!");
      setIsInterviewStarted(true); // Error aaye toh wapas resume kar sakte hain
    } finally {
              ...
              <div className="bg-slate-900/50 border border-slate-800 rounded-[2.5rem] p-8 h-full">
                ...
                <div className="space-y-10">
                  <div className="space-y-3">
                    <div className="flex justify-between text-[10px] font-black uppercase"><span className="text-slate-400">Performance</span> <span className="text-blue-500">8/10</span></div>
        ...
        <header className="w-full max-w-6xl flex justify-between items-center mb-8">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-sm tracking-widest text-slate-400 uppercase">
              {isCodingRound ? 'Coding Challenge' : 'Exam Session'}
            </span>
          </div>
        ...
        <div className="w-full max-w-7xl grid lg:grid-cols-12 gap-8 flex-1">
            ...
            <div className="bg-slate-900/40 border border-slate-800 rounded-[2rem] p-8 backdrop-blur-xl flex-1 shadow-2xl overflow-y-auto">
              ...
              {isCodingRound ? (
                ...
                <div className="bg-slate-950/60 border border-slate-800 rounded-2xl p-6 min-h-[200px] shadow-inner">
                  <p className="text-slate-600 text-[10px] font-bold mb-4 uppercase">Live Transcript</p>
          ...
          <div className="lg:col-span-5 flex flex-col gap-6">
            <div className="relative h-full min-h-[400px] bg-black rounded-[2rem] overflow-hidden border border-slate-800 shadow-2xl">
              ...
              {proctoringMessage && (
                <div className="absolute top-6 left-1/2 -translate-x-1/2 bg-red-600 px-4 py-2 rounded-full text-[10px] font-black animate-pulse">{proctoringMessage}</div>
              )}
            </div>
          </div>
        </div>
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                <label className="text-sm font-bold text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-2">
                  <Layers size={14} className="text-blue-500" /> Select Exam Category
                </label>

                {/* 🔥 Pehli 6 Categories ka Grid */}
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  <button
                    onClick={() => { setSelectedCategory(''); setShowAllCategories(false); }}
                    className={`py-3 px-4 rounded-2xl text-[10px] font-black uppercase border transition-all ${!selectedCategory ? 'bg-blue-600 border-blue-500 text-white shadow-lg' : 'bg-slate-800/40 border-slate-700 text-slate-500 hover:border-blue-500/30'}`}
                  >
                    Tech Mode
                  </button>
                  ...
                  {categories.slice(0, 5).map((cat) => (
                    <button
                      ...
                      className={`py-3 px-4 rounded-2xl text-[10px] font-black uppercase border transition-all ${selectedCategory === cat.name ? 'bg-blue-600 border-blue-500 text-white shadow-lg' : 'bg-slate-800/40 border-slate-700 text-slate-500 hover:border-blue-500/30'}`}
                    >
                      {cat.name}
                    </button>
                  ))}
                  ...
                  <button
                    ...
                    className="py-3 px-4 rounded-2xl text-[10px] font-black uppercase border border-blue-500/50 text-blue-400 bg-blue-500/5 hover:bg-blue-500/10 flex items-center justify-center gap-2 transition-all"
                  >
                    {showAllCategories ? 'Show Less' : 'More Categories +'}
                  </button>
                </div>

                {/* 🔥 DROPDOWN MENU (Sirf tab dikhega jab Show More click hoga) */}
                {showAllCategories && (
                  <div className="mt-4 p-4 bg-slate-950/80 border border-slate-800 rounded-3xl grid grid-cols-1 md:grid-cols-2 gap-2 animate-in fade-in slide-in-from-top-2 duration-300">
                    <p className="col-span-full text-[10px] font-bold text-slate-600 uppercase mb-2 px-2">All Departments</p>
                    {categories.slice(5).map((cat) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          setFile(null);
                          setJd('');
                          setShowAllCategories(false); // Select karne ke baad dropdown band
                        }}
                        className={`text-left py-3 px-5 rounded-xl text-[11px] font-bold transition-all ${selectedCategory === cat.name ? 'bg-blue-600 text-white' : 'bg-slate-900/50 text-slate-400 hover:bg-slate-800 hover:text-slate-200'}`}
                      >
                        {cat.name}
                      </button>
                    ))}
              ...
              {/* 🔥 HIDE RESUME/JD IF CATEGORY IS SELECTED */}
              {!selectedCategory && (
                <>
                  <div className="space-y-3">
              ...
              <button onClick={handleUpload} disabled={loading} className="w-full bg-blue-600 hover:bg-blue-500 py-5 rounded-[1.5rem] font-bold text-white shadow-xl flex items-center justify-center gap-3 uppercase tracking-widest text-xs">
                {loading ? <><Loader2 className="animate-spin" /> Synchronizing...</> : <><ShieldCheck size={18} /> {selectedCategory ? 'Start Exam Mode' : 'Process Technical Roadmap'}</>}
              </button>
            </div>
          </div>
